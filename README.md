...
This repository can publish a simple pollable JSON API with GitHub Pages. The API is generated from the CSV files in this repo, so it stays static and cheap to host.

Once GitHub Pages is enabled for the repository and the workflow has deployed, the main endpoints will be:

- `/api/index.json`
- `/api/manufacturers.json`
- `/api/devices.json`
- `/api/manufacturer/{manufacturer-slug}.json`
- `/api/device/{manufacturer-slug}/{device-slug}.json`

The API files are generated by `scripts/build_static_api.py`. To build them locally:

```bash
python3 scripts/build_static_api.py --output site
```

For local verification steps and `curl` examples, see `TESTING.md`.
